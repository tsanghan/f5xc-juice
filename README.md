...
4. Please also create `terraform.tfvars.tmpl` file from `terraform.tfvars.tmpl.example` and adjust to you needs, `update_ips.sh` will pull down the latest F5XC Network Cloud Reference RE IP addresses list, and create `terraform.tfvars` file from `terraform.tfvars.tmpl` template and add `ip_address_list` variable with RE IP addresses list.
